/* Variant B — "الطاقة البرتقالية" Full Orange Gradient — Bold Energy */
const BRAND = "#EA580C";
  ...
  return (
    <div style={{ width: 390, height: 280, background: "#FBFAF8", display: "flex", flexDirection: "column", fontFamily: "system-ui, sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative" }}>
        {/* Variant B: Full Orange Gradient */}
        <div style={{ position: "absolute", bottom: 50, left: "calc(12.5% + 27px)", right: 12, zIndex: 51 }}>
          <div style={{
            height: 42, display: "flex", alignItems: "center",
            padding: "0 12px 0 12px", borderRadius: 21,
            background: "linear-gradient(135deg, #ff8c38 0%, #ea580c 45%, #b83a08 100%)",
            boxShadow: "0 4px 20px rgba(234,88,12,0.50), 0 1px 0 rgba(255,255,255,0.20) inset",
            direction: "rtl",
          }}>
            {/* Right: icon + label */}
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              <div style={{ width: 26, height: 26, borderRadius: "50%", background: "rgba(255,255,255,0.18)", border: "1px solid rgba(255,255,255,0.30)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <svg width="13" height="13" fill="none" stroke="#fff" strokeWidth="2.5" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
              </div>
              <span style={{ fontSize: 13, fontWeight: 800, color: "#fff", letterSpacing: "0.4px" }}>إتمام الطلب</span>
            </div>
            <div style={{ flex: 1 }} />
            {/* Divider */}
            <div style={{ width: 1, height: 18, background: "rgba(255,255,255,0.25)", margin: "0 10px", flexShrink: 0 }} />
            {/* Left: count + price */}
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              <div style={{ background: "rgba(255,255,255,0.18)", borderRadius: 7, padding: "2px 8px" }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#fff" }}>3 قطع</span>
              </div>
              <div style={{ display: "flex", alignItems: "baseline", gap: 2 }}>
                <span style={{ fontSize: 19, fontWeight: 900, color: "#fff", letterSpacing: "-0.5px", lineHeight: 1 }}>٤٨٥</span>
                <span style={{ fontSize: 10, color: "rgba(255,255,255,0.75)" }}>ر.س</span>
              </div>
            </div>
          </div>
        </div>
