/* Variant A — "الفاخر الزجاجي" Dark Glass Morphism + Gold Price */
const BRAND = "#EA580C";
const GOLD  = "#d4af37";
      ...
      <div style={{ position: "relative" }}>
        ...
        <div style={{ position: "absolute", bottom: 50, left: "calc(12.5% + 27px)", right: 12, zIndex: 51 }}>
          <div style={{
            ...
            background: "linear-gradient(135deg,rgba(20,12,6,0.94),rgba(38,22,10,0.92))",
            backdropFilter: "blur(24px)",
            boxShadow: "0 8px 32px rgba(0,0,0,0.45), 0 1px 0 rgba(255,255,255,0.06) inset",
            direction: "rtl",
          }}>
            {/* Right: Orange circle + label */}
            <div style={{ display: "flex", alignItems: "center", gap: 9, flexShrink: 0 }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#f97316,#c2410c)", boxShadow: "0 3px 12px rgba(234,88,12,0.55)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <svg width="15" height="15" fill="none" stroke="#fff" strokeWidth="2.5" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
              </div>
              <span style={{ fontSize: 13, fontWeight: 800, color: "#fff", letterSpacing: "0.3px" }}>إتمام الطلب</span>
            </div>
            {/* Divider */}
            <div style={{ flex: 1 }} />
            <div style={{ width: 1, height: 22, background: "rgba(255,255,255,0.12)", margin: "0 10px", flexShrink: 0 }} />
            {/* Left: count + gold price */}
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ background: "rgba(249,115,22,0.15)", border: "1px solid rgba(249,115,22,0.28)", borderRadius: 8, padding: "2px 8px" }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#fb923c" }}>3 قطع</span>
              </div>
              <div style={{ display: "flex", alignItems: "baseline", gap: 2 }}>
                <span style={{ fontSize: 19, fontWeight: 900, color: GOLD, letterSpacing: "-0.5px", lineHeight: 1 }}>٤٨٥</span>
                <span style={{ fontSize: 10, color: "rgba(212,175,55,0.65)" }}>ر.س</span>
              </div>
            </div>
          </div>
        </div>
